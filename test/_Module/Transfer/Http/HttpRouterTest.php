    ...
    public function testHttpRouter_HandlesIncomingHTTPMessages()
    {
        $this
            ->simulate(function(SimulationInterface $sim) {
                ...
                $compA->on('connect', function(TransferConnectionInterface $conn) use($sim) {
                    $sim->expect('connect A');
                });
                $compA->on('message', function(TransferConnectionInterface $conn, HttpRequestInterface $message) use($sim) {
                ...
                $compB->on('connect', function(TransferConnectionInterface $conn) use($sim) {
                    $sim->expect('connect B');
                });
                $compB->on('message', function(TransferConnectionInterface $conn, HttpRequestInterface $message) use($sim) {
